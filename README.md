...
4. 在轻舟根目录执行 `php mmui-install.php`。安装器会识别轻舟版本、备份原文件并注入 MMUI Hook。
5. 登录轻舟后台，打开“系统设置-系统管理-MMUI 配置设置”。也可首次直接访问 `/admin/mmui/index` 自动执行相同修复。
6. 控制台、机器登录页切换成 MMUI 并保存。

覆盖包不会直接替换轻舟的核心控制器、Service 或虚拟化驱动。安装器目前验证 `2024083001` 至 `2026070901`，会将原文件备份到 `runtime/mmui-backup` 后执行幂等补丁。范围外版本会显示兼容提示，但仍会根据实际源码锚点尽最大努力完成安装；只有关键文件缺失、锚点无法识别或写入失败才会阻断。轻舟更新后应重新覆盖 MMUI 包并再次执行安装器。
